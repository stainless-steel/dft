...
fn complex_forward_128() {
    ...
    dft::transform(as_c64_mut(&mut data), &Plan::new(Operation::Forward, 128));
...
fn complex_inverse_128() {
    ...
    dft::transform(as_c64_mut(&mut data), &Plan::new(Operation::Inverse, 128));
...
fn real_forward_512() {
    ...
    dft::transform(&mut data, &Plan::new(Operation::Forward, 512));
    let data = dft::unpack(&data);
...
fn real_inverse_256() {
    ...
    dft::transform(&mut data, &Plan::new(Operation::Inverse, 256));
    assert::close(&data, &fixtures::TIME_DATA_256[..], 1e-14);
}
...
fn real_inverse_512() {
    let mut data = fixtures::TIME_DATA_512.to_vec();
    dft::transform(&mut data, &Plan::new(Operation::Forward, 512));
    dft::transform(&mut data, &Plan::new(Operation::Inverse, 512));
